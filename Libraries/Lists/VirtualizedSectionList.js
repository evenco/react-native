  ...
  extends React.PureComponent<DefaultProps, Props<SectionT>, State>
{
  ...
  _renderItem = ({item, index}: {item: Item, index: number}) => {
    ...
    if (infoIndex == null) {
      ...
      const sectionHeader = renderSectionHeader && renderSectionHeader({section: info.section});
      const sectionFooter = info.section.data.length == 0 && renderSectionFooter && renderSectionFooter({section: info.section});
      return (sectionHeader || sectionFooter) ? <View>{sectionHeader}{sectionFooter}</View> : null;
    } else {
