...
/**
 * Animated node that corresponds to {@code AnimatedInterpolation} from AnimatedImplementation.js, but applied to an array.
 *
 * Currently only a linear interpolation is supported on an input range of an arbitrary size.
 */
/*package*/ abstract class CompoundInterpolationAnimatedNode extends ValueAnimatedNode {
  ...
  private static double[][] fromDoubleArrayArray(ReadableArray ary) {
    double[][] res = new double[ary.size()][];
    for (int i = 0; i < res.length; i++) {
      res[i] = fromDoubleArray(ary.getArray(i));
    }
    return res;
  }
  ...
  private static double[] interpolate(
      ...
      String extrapolateLeft,
      String extrapolateRight) {
    double[] result = new double[outputMin.length];

    for (int i = 0; i < result.length; i++) {
      result[i] = interpolate(
          value,
          inputMin, inputMax,
          outputMin[i], outputMax[i],
  ...
  public CompoundInterpolationAnimatedNode(ReadableMap config) {
    // This is getting a 2-item array with min..max range for input,
    // and a 2-item array of arrays of any length for min..max output
    mInputRange = fromDoubleArray(config.getArray("inputRange"));
    mOutputRange = fromDoubleArrayArray(config.getArray("outputRange"));
  ...
  public void update() {
    ...
    mValue = getFlattenedValue(interpolate(mParent.getValue(), mInputRange, mOutputRange, mExtrapolateLeft, mExtrapolateRight));
  }

  public abstract double getFlattenedValue(double[] values);
}
